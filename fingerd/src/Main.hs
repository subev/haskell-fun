...
instance ToRow User where
  toRow (User id_ un s hd rn p) = toRow (id_, un, s, hd, rn, p)
...
{-this type that I tried to used below does not work-}
type UserRow = (Null, Text, Text, Text, Text, Text, Text)
...
createDatabase = do
  ...
  SQLite.close conn
    where {-meRow :: UserRow-}
          meRow = (Null, "callen" :: Text, "/bin/zsh" :: Text,
                   "/home/callen" :: Text, "Chris Allen" :: Text,
                   "555-123-4567" :: Text)
...
formatUser (User _ uname shell homeDir realName _) =
  BS.concat ["Login: ", e uname, "\t\t\t\t",
    "Name: ", e realName, "\n",
    "Directory: ", e homeDir, "\t\t\t",
    "Shell: ", e shell, "\n"]
  where e = encodeUtf8
